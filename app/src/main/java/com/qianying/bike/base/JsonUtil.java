...
public class JsonUtil {
    ...
    public static Object findJsonLink(String nodelink, Object rootObject) {
        if (rootObject instanceof JSONObject) {
            ...
            try {
                if (rootObject.toString().startsWith("[")) {
                    return findJsonLink(nodelink, new JSONArray(((String) rootObject)));
                } else if (((String) rootObject).startsWith("{")) {
                    return findJsonLink(nodelink, new JSONObject(((String) rootObject)));
                } else {
    ...
    private static Object getNode(JSONObject rootObject, String data)
        ...
        if (data.contains("[") && data.contains("]")) {
            int b = data.indexOf("[");
            int e = data.indexOf("]", b + 1);
            String snum = data.substring(b + 1, e);
            num = Integer.parseInt(snum);
            String fname = data.substring(0, b);
            jsonobject = rootObject.get(fname);
            if (jsonobject == null||jsonobject.toString().equals("null")) {
                return "null";
            }

            if (((JSONArray) jsonobject).length() + 1 >= num) {
                jsonobject = ((JSONArray) jsonobject).get(num);
            }
        } else {
            jsonobject = rootObject.get(data);
        }
